...
export default function PortalScreen({ navigation, route }) {
  ...
  const [watchlistSortKey, setWatchlistSortKey] = useState('handle'); // handle|return|alpha|hit
  const [watchlistSortDir, setWatchlistSortDir] = useState('asc'); // asc|desc
  ...
  // Load watchlist when tab opens or user changes
  useEffect(() => {
  ...
  const loadAnalysis = async (handle) => {
    ...
    try {
      ...
      // Call /api/analyze endpoint (thin proxy to external analyst API)
      console.log(`[Portal] Calling /api/analyze for ${handle} (${timelineMonths} months)`);
  ...
  const handleExportWatchlist = () => {
    console.log('[Portal] Exporting watchlist to CSV...');

    // Get watchlist data
    const watchlistData = watchlist && watchlist.length > 0 ? watchlist : [];

    if (watchlistData.length === 0) {
      console.log('[Portal] No watchlist to export');
      return;
    }
    ...
    console.log('[Portal] Watchlist CSV export complete');
  };

  // Get dummy watchlist accounts (5 accounts with performance data)
  const getDummyWatchlistAccounts = () => {
    return [
      {
        handle: '@elonmusk',
        name: 'Elon Musk',
  ...
  // Get dummy watchlist tweets (10 tweets with stock recommendations + 3 new posts)
  const getAllDummyWatchlistTweets = () => {
    return [
      // New posts (shown when "Show x posts" is clicked)
      {
        handle: '@elonmusk',
        name: 'Elon Musk',
        timeAgo: '5m',
        text: 'Just announced: $TSLA production numbers are through the roof this quarter. The future is here.',
        stocks: ['$TSLA'],
        replies: 45,
        retweets: 234,
        quotes: 12,
        likes: 1200,
        views: 45000,
        url: 'https://twitter.com/elonmusk/status/1234567899',
        isNew: true,
      },
      {
        handle: '@cathiedwood',
        name: 'Cathie Wood',
        timeAgo: '15m',
        text: 'Breaking: $ARKK sees record inflows. Innovation stocks are back in favor.',
        stocks: ['$ARKK'],
        replies: 23,
        retweets: 89,
        quotes: 5,
        likes: 456,
        views: 12000,
        url: 'https://twitter.com/cathiedwood/status/1234567900',
        isNew: true,
      },
      {
        handle: '@chamath',
        name: 'Chamath Palihapitiya',
        timeAgo: '30m',
        text: 'The data is clear: $NVDA continues to dominate. AI infrastructure demand is insatiable.',
        stocks: ['$NVDA'],
        replies: 67,
        retweets: 156,
        quotes: 23,
        likes: 890,
        views: 23000,
        url: 'https://twitter.com/chamath/status/1234567901',
        isNew: true,
      },
      // Regular posts
      {
        ...
        name: 'Elon Musk',
        timeAgo: '2h',
        text: 'Just added more $TSLA to my position. The future of transportation is electric. 🚗⚡',
        stocks: ['$TSLA'],
        replies: 234,
        retweets: 4382,
        quotes: 995,
      ...
      },
      {
        ...
        name: 'Cathie Wood',
        timeAgo: '4h',
        text: 'Our research shows $ARKK and $ARKQ are positioned for significant growth. Innovation is accelerating faster than most realize.',
        stocks: ['$ARKK', '$ARKQ'],
        replies: 89,
        retweets: 456,
        quotes: 123,
        likes: 2100,
        views: 125000,
        url: 'https://twitter.com/cathiedwood/status/1234567891',
      },
      {
        handle: '@chamath',
        name: 'Chamath Palihapitiya',
        timeAgo: '6h',
        text: 'The AI revolution is just beginning. $NVDA $MSFT $GOOGL are the picks and shovels. Loading up.',
        ...
        likes: 3400,
        views: 89000,
        url: 'https://twitter.com/chamath/status/1234567892',
      },
      {
        handle: '@jimcramer',
        name: 'Jim Cramer',
        timeAgo: '8h',
        text: 'Buy $AAPL on any dip. The iPhone cycle is strong and services revenue is accelerating. This is a $200 stock.',
        stocks: ['$AAPL'],
        replies: 445,
        retweets: 234,
        quotes: 67,
        likes: 1800,
        views: 45000,
        url: 'https://twitter.com/jimcramer/status/1234567893',
      },
      {
        handle: '@garyvee',
        name: 'Gary Vaynerchuk',
        timeAgo: '12h',
        text: 'The metaverse is coming. $META is building the infrastructure. Don\'t sleep on this.',
        stocks: ['$META'],
        replies: 123,
        retweets: 567,
        quotes: 89,
        likes: 2900,
        views: 67000,
        url: 'https://twitter.com/garyvee/status/1234567894',
      },
      {
        handle: '@elonmusk',
        name: 'Elon Musk',
        timeAgo: '1d',
        text: '$SPACEX is going public soon. This will be bigger than $TSLA. The space economy is the next frontier.',
        stocks: ['$SPACEX', '$TSLA'],
        replies: 890,
        retweets: 2300,
        quotes: 456,
        likes: 8700,
        views: 3200000,
        url: 'https://twitter.com/elonmusk/status/1234567895',
      },
      {
        handle: '@cathiedwood',
        name: 'Cathie Wood',
        timeAgo: '1d',
        text: '$COIN is the infrastructure play for crypto. As adoption grows, Coinbase will be the beneficiary.',
        stocks: ['$COIN'],
        replies: 234,
        retweets: 678,
        quotes: 145,
        likes: 1500,
        views: 98000,
        url: 'https://twitter.com/cathiedwood/status/1234567896',
      },
      {
        handle: '@chamath',
        name: 'Chamath Palihapitiya',
        timeAgo: '2d',
        text: 'The energy transition is happening faster than expected. $ENPH $SEDG are my top picks.',
        stocks: ['$ENPH', '$SEDG'],
        replies: 345,
        retweets: 890,
        quotes: 178,
        likes: 4200,
        views: 156000,
        url: 'https://twitter.com/chamath/status/1234567897',
      },
      {
        handle: '@jimcramer',
        name: 'Jim Cramer',
        timeAgo: '2d',
        text: '$JPM $BAC are trading at discounts. The banking sector is healthy and dividends are attractive.',
        stocks: ['$JPM', '$BAC'],
        replies: 567,
        retweets: 123,
        quotes: 45,
        likes: 987,
        views: 34000,
        url: 'https://twitter.com/jimcramer/status/1234567898',
      },
      {
        handle: '@garyvee',
        name: 'Gary Vaynerchuk',
        timeAgo: '3d',
        text: 'E-commerce is still growing. $AMZN $SHOP are the plays. The shift to online shopping is permanent.',
        stocks: ['$AMZN', '$SHOP'],
        replies: 234,
        retweets: 456,
        quotes: 78,
        likes: 1200,
        views: 56000,
        url: 'https://twitter.com/garyvee/status/1234567899',
        isNew: false,
      },
    ];
  };

  // Get visible tweets based on showNewPosts state
  const getDummyWatchlistTweets = () => {
    const allTweets = getAllDummyWatchlistTweets();
    if (showNewPosts) {
      return allTweets;
    }
    return allTweets.filter(tweet => !tweet.isNew);
  };

  // Get count of new posts
  const getNewPostsCount = () => {
    return getAllDummyWatchlistTweets().filter(tweet => tweet.isNew).length;
  };
      ...
      <ScrollView style={styles.mainContent} contentContainerStyle={styles.mainContentContainer}>
                    ...
                    <View style={styles.profileLeft}>
                      ...
                      <View style={styles.profileInfo}>
                        ...
                        >
                          ...
                          <Text style={styles.favouriteButtonText}>Add to Watchlist</Text>
                        </Pressable>
                      </View>
                    </View>
        ...
        {activeTab === 'watchlist' && (
          <View style={styles.contentSection}>
            <View style={[styles.watchlistContainer, isMobile && styles.watchlistContainerMobile]}>
            {/* Left Side - Twitter Feed */}
            <View style={[styles.watchlistFeed, isMobile && styles.watchlistFeedMobile]}>
              <View style={styles.watchlistTitleContainer}>
                <Text style={styles.watchlistFeedTitle}>Latest Recommendations</Text>
              </View>
              <ScrollView style={styles.feedScrollView} showsVerticalScrollIndicator={true}>
                {!showNewPosts && getNewPostsCount() > 0 && (
                  <Pressable 
                    style={styles.showNewPostsButton}
                    onPress={() => {
                      setShowNewPosts(true);
                      setNewPostsCount(0);
                    }}
                  >
                    <Text style={styles.showNewPostsText}>
                      Show {getNewPostsCount()} posts
                    </Text>
                  </Pressable>
                )}
                {getDummyWatchlistTweets().map((tweet, index) => (
                  <View key={index} style={styles.tweetCard}>
                    {/* Tweet Header */}
                    <View style={styles.tweetHeader}>
                      <View style={styles.tweetAvatar}>
                        <Text style={styles.tweetAvatarText}>
                          {tweet.handle.charAt(1).toUpperCase()}
                        </Text>
                      </View>
                      <View style={styles.tweetAuthorContainer}>
                        <View style={styles.tweetAuthorRow}>
                          <Text style={styles.tweetAuthorName}>{tweet.name}</Text>
                          <Text style={styles.tweetVerifiedBadge}>✓</Text>
                        </View>
                        <Text style={styles.tweetAuthorHandle}>{tweet.handle}</Text>
                      </View>
                    </View>
                    ...
                    {/* Stock Mentions */}
                    {tweet.stocks && tweet.stocks.length > 0 && (
                      <View style={styles.tweetStocks}>
                        {tweet.stocks.map((stock, idx) => (
                          <Text key={idx} style={styles.stockMention}>{stock}</Text>
                        ))}
                      </View>
                    )}
                    
                    {/* Timestamp and Views */}
                    <View style={styles.tweetMetadata}>
                      <Text style={styles.tweetDate}>{tweet.timeAgo}</Text>
                      <Text style={styles.tweetMetadataSeparator}> · </Text>
                      <Text style={styles.tweetViews}>
                        {typeof tweet.views === 'number' && tweet.views >= 1000000
                          ? (tweet.views / 1000000).toFixed(1) + 'M'
                          : tweet.views >= 1000
                    ...
                    <View style={styles.tweetEngagement}>
                      <View style={styles.tweetEngagementItem}>
                        <Text style={styles.tweetEngagementNumber}>
                          {typeof tweet.retweets === 'number' && tweet.retweets >= 1000 
                      ...
                      <View style={styles.tweetEngagementItem}>
                        <Text style={styles.tweetEngagementNumber}>
                          {typeof tweet.quotes === 'number' && tweet.quotes >= 1000 
                            ? (tweet.quotes / 1000).toFixed(1) + 'K' 
                            : tweet.quotes || 0}
                        </Text>
                        <Text style={styles.tweetEngagementLabel}> Quotes</Text>
                      </View>
                      <View style={styles.tweetEngagementItem}>
                        <Text style={styles.tweetEngagementNumber}>
                          {typeof tweet.likes === 'number' && tweet.likes >= 1000 
            ...
            <View style={[styles.watchlistTable, isMobile && styles.watchlistTableMobile]}>
              <View style={styles.watchlistTitleContainer}>
                <View style={styles.watchlistTableHeader}>
                  <Text style={styles.watchlistTableTitle}>Performance Comparison</Text>
                  <Pressable onPress={handleExportWatchlist}>
                    <ExportIcon size={18} color="#697386" />
                  </Pressable>
                </View>
              </View>
              
              <ScrollView style={styles.comparisonTableScroll} showsVerticalScrollIndicator={false}>
                <View style={styles.comparisonTable}>
                  {/* Table Header */}
                  <View style={styles.comparisonTableHeader}>
                    <Text style={[styles.comparisonTableHeaderCell, { flex: 2.5, minWidth: 180 }]}>Account</Text>
                    <Text style={[styles.comparisonTableHeaderCell, { flex: 1 }]}>Return</Text>
                    <Text style={[styles.comparisonTableHeaderCell, { flex: 1 }]}>Alpha</Text>
                    <Text style={[styles.comparisonTableHeaderCell, { flex: 1 }]}>Win Rate</Text>
                    <Text style={[styles.comparisonTableHeaderCell, { flex: 1 }]}>Trades</Text>
                  </View>

                  {/* Table Rows */}
                  {getDummyWatchlistAccounts().map((account, idx) => (
                  <Pressable
                    key={idx}
                    style={[
                      styles.comparisonTableRow,
                      idx % 2 === 1 && styles.comparisonTableRowZebra
                    ]}
                    onPress={() => {
                      setTwitterHandle(account.handle);
                      setActiveTab('analyzer');
                      loadAnalysis(account.handle.replace('@', ''));
                    }}
                  >
                    <View style={[styles.comparisonTableCell, { flex: 2.5, flexDirection: 'row', alignItems: 'center', gap: 8, minWidth: 180 }]}>
                      ...
                      <View style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <Text style={styles.accountHandle} numberOfLines={1}>{account.handle}</Text>
                      </View>
                    </View>
                    <Text style={[
                      styles.comparisonTableCell,
                      { flex: 1, color: account.avgReturn >= 0 ? '#1E8E3E' : '#D93025', fontWeight: '600' }
                    ]}>
                      {account.avgReturn >= 0 ? '+' : ''}{account.avgReturn.toFixed(1)}%
                    </Text>
                    <Text style={[
                      styles.comparisonTableCell,
                      { flex: 1, color: account.alpha >= 0 ? '#1E8E3E' : '#D93025' }
                    ]}>
                      {account.alpha >= 0 ? '+' : ''}{account.alpha.toFixed(1)}%
                    </Text>
                    <Text style={[styles.comparisonTableCell, { flex: 1 }]}>
